...
const TestScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setTimeLeft((prev) => {
        if (prev <= 1) {
          clearInterval(timer);
          setTimerExpired(true); // Set timerExpired to true when time runs out
        }
        return prev - 1;
      });
    }, 1000);
  ...
  useEffect(() => {
    if (timerExpired) {
      handleSubmit(); // Call handleSubmit when timer expires
    }
  }, [timerExpired]);
  ...
  const finalizeSubmission = async () => {
    ...
    const submissionData = {
      ...
      timeTaken: `${Math.floor((TOTAL_TIMER_DURATION - timeLeft) / 60)} min ${(TOTAL_TIMER_DURATION - timeLeft) % 60
        } sec`,
  ...
  const submitTest = async (data) => {
    // console.log('Submitting test data:', JSON.stringify(data, null, 2));
